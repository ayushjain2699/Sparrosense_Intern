...
FLAGS = flags.FLAGS
model_name = "model_motion_statistics.ckpt"

momentum = 0.9


def train():
    img_input = tf.placeholder(tf.float32, shape=(None, 16, 112, 112, 3),name = "img_input")
    y_target = tf.placeholder(tf.float32, shape=(None,FLAGS.dimension),name = "y_target")
    ...
    with tf.Session() as sess:
        train_writer = tf.summary.FileWriter('./visual_logs/train_SSL', sess.graph)
        init = tf.global_variables_initializer()
        sess.run(init)

        for i in range(1,FLAGS.max_iter+1):
